...
const FormBlockImage = ({ propertyId, isEditMode }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 flex flex-col gap-4 shadow-sm animate-fade-in text-xs font-light text-slate-700 dark:text-slate-300">
      <ImageUploadInput handleFileChange={handleFileChange} selectedFile={selectedFile} handleUploadClick={handleUploadClick}/>
      
      {propertyImages?.length > 0 && (
        <ImageGalleryGrid propertyImages={propertyImages} triggerLightboxView={triggerLightboxView} handleSetCover={handleSetCover} handleDeleteImage={handleDeleteImage}/>
      )}
