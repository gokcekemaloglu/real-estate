    ...
    <div className="flex flex-col gap-6 animate-fade-in max-w-4xl text-xs font-light text-slate-700 dark:text-slate-300">
      ...
      <form onSubmit={formik.handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 flex flex-col gap-4 shadow-sm">
          <h3 className="text-xs uppercase tracking-widest text-brand-gold font-medium border-b border-slate-100 dark:border-slate-800/60 pb-2">1. Başlık & Mimari Tanım</h3>
          
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] uppercase tracking-wider text-slate-400 font-medium">İlan Başlığı</label>
            <input 
              type="text" name="title" placeholder="Seyhan'da Havuzlu Lüks Müstakil Villa..."
            ...
            />
            {hasTitleError && <span className="text-[11px] text-red-500 font-light mt-1">{formik.errors.title}</span>}
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] uppercase tracking-wider text-slate-400 font-medium">Mimari Açıklama</label>
            <textarea 
              name="description" rows="4" placeholder="Mülkün tüm lüks konfor ayrıntılarını buraya işleyin..."
              value={formik.values.description} onChange={formik.handleChange} onBlur={formik.handleBlur}
              className={`input-premium bg-slate-50/50 dark:bg-slate-950/20 p-3 leading-relaxed whitespace-pre-line ${hasDescError ? "border-red-500 focus:border-red-500" : ""}`}
            />
            {hasDescError && <span className="text-[11px] text-red-500 font-light mt-1">{formik.errors.description}</span>}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 grid grid-cols-1 md:grid-cols-3 gap-6 shadow-sm">
          <h3 className="md:col-span-3 text-xs uppercase tracking-widest text-brand-gold font-medium border-b border-slate-100 dark:border-slate-800/60 pb-2">2. Durum & Fiyatlandırma</h3>

          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] uppercase tracking-wider text-slate-400 font-medium">Fiyat (₺)</label>
            <input 
              type="number" name="price" placeholder="4500000"
            ...
            />
            {formik.touched.price && formik.errors.price && <span className="text-[11px] text-red-500 font-light mt-1">{formik.errors.price}</span>}
          </div>

          <FormSelectField label="İlan Durumu" name="listingType" options={listingTypeOptions} formik={formik} />
          <FormSelectField label="Mülk Kategorisi" name="propertyCategory" options={categoryOptions} formik={formik} />
        </div>
