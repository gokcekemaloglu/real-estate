...
const useFetchData = () => {
  ...
  const fetchData = async ({ endpoint, stateKey, sliceActions, page = 1, limit = 12, query = "" }) => {
    ...
    try {
      ...
      // A: Inject core records data array directly into state mapping location (e.g., state.properties)
      dispatch(sliceActions.setData({ endpoint: stateKey, data: data?.data }));

      // B: Map standard pagination meta blocks into dedicated key variables (e.g., state.propertiesDetails)
